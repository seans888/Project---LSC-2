 ...
 */
!function ($) {
    var pluginName = "yiiDynamicForm", regexID = /^(.+?)([-\d-]{1,})(.+)$/i, regexName = /(^.+?)([\[\d{1,}\]]{1,})(\[.+\]$)/i;
    ...
    var events = {
        ...
        return $template.find("div[data-dynamicform]").each(function () {
            var widgetOptions = eval($(this).attr("data-dynamicform"));
            ...
            var e = $(this).attr("name"), t = $template.find('input[type="hidden"][name="' + e + '"]').first();
            t && ($(this).val(1), t.val(0))
        }), $template
        ...
        if (count > 1) {
            ...
            var level = _getLevel($elem.closest("." + widgetOptions.widgetContainer)), widgetOptionsRoot = _getWidgetOptionsRoot(widgetOptions), identifiers = _createIdentifiers(level);
        ...
        var widgetOptions = eval($elem.closest("div[data-dynamicform]").attr("data-dynamicform")), id = $elem.attr("id"), newID = id;
        if (void 0 !== id) {
        ...
        $(widgetOptionsRoot.widgetBody).find("input, textarea, select").each(function () {
            ...
            if (void 0 !== id && void 0 !== name) {
                ...
                if (matches && 4 === matches.length) {
                    matches[2] = matches[2].substring(1, matches[2].length - 1);
                    var level = _getLevel($(this)), identifiers = _createIdentifiers(level - 1), baseID = matches[1] + "-" + identifiers.join("-") + "-" + matches[3], attribute = $("#" + currentWidgetOptions.formId).yiiActiveForm("find", baseID);
                    _fixFormValidatonInput(currentWidgetOptions, attribute, id, name)
                }
            }
        })
    }, _restoreSpecialJs = function (widgetOptions) {
        var widgetOptionsRoot = _getWidgetOptionsRoot(widgetOptions), $hasDatepicker = $(widgetOptionsRoot.widgetItem).find("[data-krajee-datepicker]");
        ...
        $hasDepdrop.length > 0 && $hasDepdrop.each(function () {
            $(this).removeData().off(), $(this).unbind();
            var configDepdrop = eval($(this).attr("data-krajee-depdrop")), inputID = $(this).attr("id"), matchID = inputID.match(regex);
        ...
        $hasSelect2.length > 0 && $hasSelect2.each(function () {
            ...
            $(this).select2("destroy"), $.when($("#" + id).select2(configSelect2)).done(initSelect2Loading(id)), $("#" + id).on("select2-open", function () {
                initSelect2DropStyle(id)
            }), $(this).attr("data-krajee-depdrop") && ($(this).on("depdrop.beforeChange", function (e, i, v) {
                var configDepdrop = eval($(this).attr("data-krajee-depdrop")), loadingText = configDepdrop.loadingText ? configDepdrop.loadingText : "Loading ...";
                $("#" + id).select2("data", {text: loadingText})
            }), $(this).on("depdrop.change", function () {
