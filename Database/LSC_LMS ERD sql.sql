...
CREATE TABLE IF NOT EXISTS `lsc_lms`.`course` (
  ...
  `course_description` VARCHAR(45) NULL COMMENT '',
  `time_created` TIMESTAMP(6) NULL COMMENT '',
...
CREATE TABLE IF NOT EXISTS `lsc_lms`.`task` (
  ...
  `task_type` VARCHAR(45) NULL COMMENT '',
...
CREATE TABLE IF NOT EXISTS `lsc_lms`.`ann_calendar` (
  ...
  CONSTRAINT `fk_ann_calendar_task1`
    ...
    REFERENCES `lsc_lms`.`task` (`id` , `course_id` , `course_employee_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `lsc_lms`.`choice` (
  ...
  `is_correct` VARCHAR(45) NOT NULL COMMENT '',
...
CREATE TABLE IF NOT EXISTS `lsc_lms`.`result` (
  `feedback` VARCHAR(45) NOT NULL COMMENT '',
  ...
  PRIMARY KEY (`stud_answer_choice_id`, `stud_answer_choice_question_id`, `stud_answer_student_id`, `feedback`)  COMMENT '',
